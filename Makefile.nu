...
CPP_FLAGS ?= -std=c++11 ${COMPILE_FLAGS} ${COMMON_FLAGS}
...
bin/lip: << C_FLAGS CPP_FLAGS CC LIBLIP LIBLIP_EXTRA_FLAGS CLEAR_ENV
	...
	${NUMAKE} exe:$@ \
		...
		c_flags="${C_FLAGS} ${LIBLIP_EXTRA_FLAGS} -Isrc/dbg -Ideps/linenoise -Ideps/cargo" \
		linker="${CC}" \
		libs="bin/liblinenoise.a bin/libcargo.a bin/libdbg.a bin/libcmp.a ${LIBLIP}"
...
bin/libcmp.a: << CLEAR_ENV ASAN_COMPILE_FLAGS UBSAN_COMPILE_FLAGS LTO_FLAGS
	${CLEAR_ENV}
	${NUMAKE} static-lib:$@ \
		c_flags="-std=c99 ${LTO_FLAGS} ${ASAN_COMPILE_FLAGS} ${UBSAN_COMPILE_FLAGS}" \
